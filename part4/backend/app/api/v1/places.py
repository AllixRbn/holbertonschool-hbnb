...
place_create_model = api.model('PlaceCreate', {
    ...
    'city': fields.String(description='City where the place is located'),
    'image_url': fields.String(description='Image URL for the place'),
...
class PlaceResource(Resource):
    ...
    def delete(self, place_id):
        ...
        if not place:
            return {"error": "Place not found"}, 404
        current_user = get_jwt_identity()
        if place.owner.id != current_user and not _is_admin():
...
class PlaceAmenityList(Resource):
    ...
    @api.expect(api.model('PlaceAmenityUpdate', {
        'amenity_ids': fields.List(
            ...
            description='List of amenity IDs to associate')
    }), validate=True)
    ...
    def post(self, place_id):
        ...
        try:
            updated_place = facade.add_amenities_to_place(
